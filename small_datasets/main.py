...
def evaluate_iteration(triple):

    (pl_f, pi_f), (pv_f, pj_f), (pu_f, pk_f) = triple

    pl_r = tuple(Rational(x) for x in pl_f)
    pi_r = tuple(Rational(x) for x in pi_f)
    pv_r = tuple(Rational(x) for x in pv_f)
    pj_r = tuple(Rational(x) for x in pj_f)
    pu_r = tuple(Rational(x) for x in pu_f)
    pk_r = tuple(Rational(x) for x in pk_f)

    signYapL, depthYapL = evaluation.evaluateTable(
        pExpressionsYapLex,
        pl, pi, pv, pj, pu, pk,
        pl_r, pi_r, pv_r, pj_r, pu_r, pk_r
    )

    signYapT, depthYapT = evaluation.evaluateTable(
        pExpressionsYapTotal,
        pl, pi, pv, pj, pu, pk,
        pl_r, pi_r, pv_r, pj_r, pu_r, pk_r
    )

    signSoS, depthSoS = evaluation.evaluateTable(
        pExpressionsSoS,
        pl, pi, pv, pj, pu, pk,
        pl_r, pi_r, pv_r, pj_r, pu_r, pk_r
    )
...
with ProcessPoolExecutor() as executor:
    results = list(tqdm(
        executor.map(evaluate_iteration, segment_triples),
        total=len(segment_triples),
